<template>
    <div>
        ...
        <!-- 表格组件
            data:表格组件将展示的数据--数据类型
            label:显示标题

            elementui中table组件，展示的数据是以一列一列进行展示数据
            prop:对应列内容的字段名
         -->
        ...
        <!-- 
            ...
            page-size:代表可以设置每一页展示多少条数据
            layout:可以实现分页器布局

         -->
        <el-pagination style="margin-top:20px;textAlign:center" :current-page="6" :total="total" :page-size="limit"
        ...
        <!-- 
            对话框
            :visible.sync控制对话框是否显示隐藏
            Form 组件提供了表单验证的功能，只需要通过 rules 属性传入约定的验证规则，并将 Form-Item 的 prop 属性设置为需校验的字段名即可。
         -->
        <el-dialog :title="tmForm.id?'修改品牌':'添加品牌'" :visible.sync="dialogFormVisible">
            <!-- from表单 :model属性，这个属性的作用是把表单数据收集到那个对象身上，将来表单验证，也需要这个属性 -->
...
export default {
    ...
    methods: {
        ...
        async getPageList(pager = 1) {
            ...
            // 获取品牌列表的接口
            //向服务器发送请求时,函数需要带参数,在data中初始化两个字段,代表给服务器传递参数
